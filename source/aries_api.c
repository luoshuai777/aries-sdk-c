        ...
        uint8_t recoveryAddr)
{
        ...
        ((device->fwVersion.major == 1) && (device->fwVersion.minor < 24)))
    {
        ...
        device->fwUpdateMmAssistBlockSizeBytes = ARIES_EEPROM_BLOCK_WRITE_SIZE_WIDE;
        device->fwUpdateMmAssistBaseAddr = ARIES_EEPROM_BLOCK_BASE_ADDR_WIDE;
        device->fwUpdateMmAssistCmdModifier = ARIES_EEPROM_BLOCK_CMD_MODIFIER_WIDE;
    }
    else
    {
        // Use optimized FW update parameters
        device->fwUpdateMmAssistBlockSizeBytes = ARIES_EEPROM_BLOCK_WRITE_SIZE_NOWIDE;
        device->fwUpdateMmAssistBaseAddr = ARIES_EEPROM_BLOCK_BASE_ADDR_NOWIDE;
        device->fwUpdateMmAssistCmdModifier = ARIES_EEPROM_BLOCK_CMD_MODIFIER_NOWIDE;
    }
 ...
 */
AriesErrorType ariesSetHwReset(
        AriesDeviceType* device,
        uint8_t reset)
{
    AriesErrorType rc;
 ...
 */
AriesErrorType ariesSetI2CMasterReset(
        AriesDeviceType* device,
        uint8_t reset)
{
    AriesErrorType rc;
 ...
 * Update the FW image in the EEPROM connected to the Retimer.
 */
AriesErrorType ariesFirmwareUpdateProgress(
        AriesDeviceType* device,
        uint8_t* percentComplete)
{
    *percentComplete = 100 * device->fwUpdateProg / (ARIES_FW_UPDATE_PROGRESS_COMPLETE - ARIES_FW_UPDATE_PROGRESS_START + 1);
        ...
        bool legacyMode)
{
    ...
    {
        eepromEnd += 8;
        eepromWriteDelta = eepromEnd % device->fwUpdateMmAssistBlockSizeBytes;
        if (eepromWriteDelta)
        {
            eepromEnd += device->fwUpdateMmAssistBlockSizeBytes - eepromWriteDelta;
        }
        ...
        addrDiffDelta = addrDiff % device->fwUpdateMmAssistBlockSizeBytes;
        if (addrDiffDelta)
        {
            addrDiff += device->fwUpdateMmAssistBlockSizeBytes - addrDiffDelta;
        }
    }
        ...
        while (addr < eepromEnd)
        {
            ...
            while (burst < ARIES_EEPROM_PAGE_SIZE)
            {
                ...
                if (addr == addrFlag)
                {
                    ...
                    // addrBurst
                    rc = ariesI2CMasterMultiBlockWrite(device,
        ...
        bool legacyMode)
{
    ...
    if ((!legacyMode) && mainMicroAssist)
    {
        ...
        for (addr = 0; addr < eepromEnd; addr+=device->fwUpdateMmAssistBlockSizeBytes)
        {
            addrMSB = addr / 65536;
            addrI2C = addr % 65536;
            ...
            int byteIdx;
            rewriteFlag = false;
            for (byteIdx = 0; byteIdx < device->fwUpdateMmAssistBlockSizeBytes; byteIdx++)
            {
            ...
            if (rewriteFlag)
            {
                rc = ariesI2CMasterSendAddress(device->i2cDriver,
                        (addr+device->fwUpdateMmAssistBlockSizeBytes));
                CHECK_SUCCESS(rc);
            }
        ...
        uint8_t* image)
{
    ...
    for (addr = 0; addr < eepromEnd; addr+=ARIES_EEPROM_BANK_SIZE)
    {
        ...
        if (eepromBlockEndFlag)
        {
            ...
            rc = ariesI2CMasterGetChecksum(device,
        ...
        AriesLinkType* link)
{
    ...
    for (laneIndex = 0; laneIndex < link->state.width; laneIndex++)
    {
        ...
        rc = ariesGetMinFoMVal(link->device, upstreamSide, pathID, lane,
                ARIES_PMA_RAWLANE_DIG_PCS_XF_RX_ADAPT_FOM, dataWord);
        CHECK_SUCCESS(rc);
        ...
        AriesLinkType* link)
{
    ...
    {
        if (link->config.startLane == (bifurcationModes[bifMode].links[linkIdx].startLane))
        {
...
{
    AriesErrorType rc;
    // Set csr.misc.glb_err_msk_reg2[7]=1 to trigger FW reload on next SBR. Aries FW
    // will self-clear bit 7 once SBR-triggered FW reload starts.
    uint8_t dataByte[1];
    dataByte[0] = 0x80;
 ...
 */
AriesErrorType ariesGetI2CBusClearEventStatus(
        AriesDeviceType* device,
        int* status)
{
    AriesErrorType rc;
 ...
 */
AriesErrorType ariesClearI2CBusClearEventStatus(
        AriesDeviceType* device)
{
    AriesErrorType rc;
    uint8_t dataByte[1];
