 ...
 */
AriesErrorType ariesReadWriteWordPmaLaneMainMicroIndirect(AriesI2CDriverType* i2cDriver, int side,
                                                          int quadSlice, int quadSliceLane, uint16_t pmaAddr,
                                                          int offset, uint16_t value, int width)
{
    AriesErrorType rc;
    uint8_t dataWord[2];

    rc = ariesReadWordPmaLaneMainMicroIndirect(i2cDriver, side, quadSlice, quadSliceLane,
                                               pmaAddr, dataWord);
    CHECK_SUCCESS(rc)
    ...
    if (value >= (1 << width))
    {
        ASTERA_WARN("Trying to write a value greater than the width of the bitfield. Truncation occurred.");
    }
    ...
    rc = ariesWriteWordPmaLaneMainMicroIndirect(i2cDriver,side, quadSlice, quadSliceLane,
                                                pmaAddr, dataWord);
    CHECK_SUCCESS(rc)
