        ...
        uint8_t* values)
{
    ...
    // IC Data command
    dataByte[0] = 0x10;
    rc = ariesWriteByteData(device->i2cDriver, ARIES_I2C_MST_IC_CMD_ADDR, dataByte);
    CHECK_SUCCESS(rc);

    // Prepare Flag Byte
    dataByte[0] = 0;
    rc = ariesWriteByteData(device->i2cDriver, ARIES_I2C_MST_DATA1_ADDR, dataByte);
    CHECK_SUCCESS(rc);
    ...
    rc = ariesWriteByteData(device->i2cDriver, ARIES_I2C_MST_DATA0_ADDR, dataByte);
    CHECK_SUCCESS(rc);
    dataByte[0] = 1;
    rc = ariesWriteByteData(device->i2cDriver, ARIES_I2C_MST_CMD_ADDR, dataByte);
    CHECK_SUCCESS(rc);
    dataByte[0] = addr7To0;
    rc = ariesWriteByteData(device->i2cDriver, ARIES_I2C_MST_DATA0_ADDR, dataByte);
    CHECK_SUCCESS(rc);
    ...
    int numIters = numBytes/device->fwUpdateMmAssistBlockSizeBytes;
    int iterIdx;
    int numBlocks = device->fwUpdateMmAssistBlockSizeBytes / 4; // 4-byte blocks
    int blockIdx;
    int byteIdx;
    ...
    for (iterIdx = 0; iterIdx < numIters; iterIdx++)
    {
        ...
        cmd = cmd | device->fwUpdateMmAssistCmdModifier;
        ...
        for (blockIdx = 0; blockIdx < numBlocks; blockIdx++)
        {
            ...
            rc = ariesWriteBlockData(device->i2cDriver, device->fwUpdateMmAssistBaseAddr + 4*blockIdx,
                    4, dataBytes);
            CHECK_SUCCESS(rc);
        }
        ...
        {
            rc = ariesReadByteData(device->i2cDriver, ARIES_MM_EEPROM_ASSIST_CMD_ADDR,
                    dataByte);
            CHECK_SUCCESS(rc);
            if (dataByte[0] == 0)
            {
        ...
        if (mmBusy)
        {
            ASTERA_TRACE("ERROR: Main Micro busy writing data block to EEPROM. Did not commit write");
            return ARIES_EEPROM_MM_STATUS_BUSY;
        }

        offset += device->fwUpdateMmAssistBlockSizeBytes;
    }
        ...
        uint8_t* dataBytes)
{
    ...
    dataByte[0] = ARIES_MM_EEPROM_READ_END_CODE | device->fwUpdateMmAssistCmdModifier;
    rc = ariesWriteByteData(device->i2cDriver, ARIES_MM_EEPROM_ASSIST_CMD_ADDR,
            dataByte);
    CHECK_SUCCESS(rc);
    ...
    int byteIdx;
    int dataByteIdx;
    for (byteIdx = 0; byteIdx < device->fwUpdateMmAssistBlockSizeBytes; byteIdx+=4)
    {
        rc = ariesReadBlockData(device->i2cDriver,
                (device->fwUpdateMmAssistBaseAddr+byteIdx), 4, data);
        CHECK_SUCCESS(rc);
        ...
        uint32_t* checksum)
{
    ...
    if (blockEnd != 0)
    {
        ...
        dataBytes[2] = 0;
        dataBytes[3] = 0;
        rc = ariesWriteBlockData(device->i2cDriver, device->fwUpdateMmAssistBaseAddr,
            4, dataBytes);
        CHECK_SUCCESS(rc);
    ...
    bool mmBusy = true;
    dataByte[0] = commandCode | device->fwUpdateMmAssistCmdModifier;
    rc = ariesWriteByteData(device->i2cDriver, ARIES_MM_EEPROM_ASSIST_CMD_ADDR,
            dataByte);
    CHECK_SUCCESS(rc);
    ...
    if (mmBusy)
    {
        ASTERA_ERROR("ERROR: Main Micro busy calculating block checksum. Read data not ready");
        return ARIES_EEPROM_MM_STATUS_BUSY;
    }
    ...
    *checksum = 0;
    ...
    for (byteIdx = 0; byteIdx < ARIES_EEPROM_MM_BLOCK_CHECKSUM_WRITE_SIZE; byteIdx+=4)
    {
        rc = ariesReadBlockData(device->i2cDriver,
                (device->fwUpdateMmAssistBaseAddr+byteIdx), 4, dataBytes);
        CHECK_SUCCESS(rc);
        for (dataByteIdx = 0; dataByteIdx < 4; dataByteIdx++)
        {
        ...
        uint16_t* dpllFreq)
{
    ...
    uint8_t dataWord[2];
    rc = ariesReadWordPmaLaneMainMicroIndirect(link->device->i2cDriver, side,
            pmaNum, pmaLane, ARIES_PMA_LANE_DIG_RX_DPLL_FREQ,
            dataWord);
    CHECK_SUCCESS(rc);
        ...
        uint8_t* mem)
{
    ...
    numBytesRead = fread(mem, 1, ARIES_EEPROM_NUM_BYTES, fin);

    if (numBytesRead != ARIES_EEPROM_NUM_BYTES)
    {
        ASTERA_ERROR("We did not read out the expected ARIES_EEPROM_NUM_BYTES: %d bytes from file '%s' was %d",
                ARIES_EEPROM_NUM_BYTES, filename, numBytesRead);
        fclose(fin);
        return ARIES_FAILURE;
    }
    else if (ferror(fin))
    {
        ASTERA_ERROR("There was some error in reading from file '%s'", filename);
        fclose(fin);
        return ARIES_FAILURE;
    }
 ...
 */
AriesErrorType ariesReadBlockDataForceError(
        AriesI2CDriverType* i2cDriver,
        uint32_t address,
        uint8_t numBytes,
        uint8_t* values)
